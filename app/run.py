...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MBA 15min - analise academica direta e PDF sem parser.")
    ...
    group.add_argument("--serve-feedback", action="store_true", help="Inicia servidor HTTP de confirmacao.")
    group.add_argument("--mark-last-read", action="store_true", help="Marca a leitura pendente atual como lida.")
    group.add_argument("--mark-read-token", metavar="TOKEN", help="Confirma leitura pelo token.")
    parser.add_argument("--force", action="store_true", help="Ignora idempotencia do dia.")
    parser.add_argument("--day", type=int, help="Dia especifico (1..365).")
    ...
    parser.add_argument("--catalog-year", type=int, help="Ano de referencia do catalogo anual.")
    parser.add_argument("--rebuild-catalog", action="store_true", help="Forca reavaliacao anual da lista de livros.")
...
def run() -> int:
    ...
    catalog_year = args.catalog_year or today.year
    ...
    logger.info("Arquivo markdown: %s", artifacts.markdown_path)
    logger.info("Arquivo texto: %s", artifacts.text_path)
    logger.info("PDF gerado: %s (palavras aproximadas: %s)", artifacts.pdf_path, artifacts.lesson.word_count())
    logger.info("Link YouTube (%s): %s", artifacts.youtube_reference.source, artifacts.youtube_reference.url)
    ...
    if not manual_day and not read_confirmation_url:
        logger.info("Link de confirmacao nao incluido. Configure READ_CONFIRM_BASE_URL ou use --mark-last-read.")
    if manual_day:
...
def _mark_latest_pending_read(state_file) -> int:
...
def _mark_read_by_token(state_file, token: str) -> int:
